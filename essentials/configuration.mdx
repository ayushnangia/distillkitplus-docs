...
Configuration for distillation runs in `distillKitPlus` is managed via JSON files. These files define all the necessary parameters for setting up datasets, models, tokenizers, training processes, and distillation-specific settings.

## Configuration File Structure

The configuration is organized into logical sections that correspond to different aspects of the distillation process. Here's a visual breakdown of the configuration structure:

```mermaid
graph TD
    A[config.json] --> B[project_name]
    A --> C[dataset]
    A --> D[models]
    A --> E[tokenizer]
    A --> F[training]
    ...
    D --> D1[teacher]
    D --> D2[student]
    D --> D3[student_adapter]
    D --> D4[teacher_adapter]
    D --> D5[teacher_vocab_size]
    
    F --> F1[output_dir]
    F --> F2[num_train_epochs]
    F --> F3[per_device_train_batch_size]
    F --> F4[...]
  ...
  <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
    ...
    <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
      ...
      <tr>
        ...
        <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-300">object</td>
        <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-300">Additional parameters for specific loss types</td>
      </tr>
    </tbody>
  </table>
</div>
...
```json
{
  ...
  "dataset": {
    ...
    "split": "train",
    "logits_file": "logits/llama-3.1-70b-alpaca.tfrecord",
    "num_samples": 10000,
    "select_range": null,
...
1. **Memory Optimization:**
   ...
   - Use pre-computed logits for large teacher models
...
3. **Result Quality:**
   - Try different loss types (`distillation.loss_type`)
