...
<Note>
  If you are providing a path for `"logits_file"`, you must first generate the teacher logits by following the steps in the [Generating Teacher Logits](./running-distillation#generating-teacher-logits-optional) section.
</Note>
...
If you plan to run your jobs using Modal, you'll need to upload your configuration files (e.g., `my_config.json` or `logit_generation_config.json`) to a Modal Volume (e.g., `distillation-volume`). You should also upload the Accelerate and DeepSpeed configuration files. The corresponding Modal scripts (`scripts/modal/distill_logits.py` or `scripts/modal/generate_logits.py`) must be configured to mount this volume and access the files from the volume path.
</Note>
